...
// compute perpendicular vector to _base_vector that is closest to the given _vector
    ...
    switch(planner_mode_) {
        ...
        case PlannerMode::looking_for_object:
            if (got_first_object_pose_ && got_first_platform_pose_ && got_first_cylinder_holder_pose_) {
                ...
                eef_pos_des_ = object_pos_ + (offset_dist_ + 0.5*cylinder_height_) * object_plane_normal_.normalized(); 
        ...
        case PlannerMode::gripping_object:
            ...
            if (gripper_status_ == GripperStatus::gripped) {
                std::lock_guard<std::mutex> lock(mutex_);
                eef_pos_des_ = eef_pos_des_ + lift_surface_dist_ * object_plane_normal_.normalized();  //assumes inertial z-axis downwards
        ...
        case PlannerMode::lift_from_surface:
            ...
            // approaching platform with respect to cylinder frame
            if (traj_status_ == TrajStatus::reached_goal) {
                got_first_object_pose_ = false;
        ...
        case PlannerMode::read_new_cylinder: // correct any offset of cylinder from ideal gripping position
            if (got_first_object_pose_) {
                ...
                eef_pos_des_ = dropping_pos_ + (offset_dist_ + 0.5*cylinder_height_) * dropping_plane_normal_.normalized();
