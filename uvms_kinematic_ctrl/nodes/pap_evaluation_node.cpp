...
void PaPEvaluationNode::onTrajStatus(const std_msgs::msg::Int64::SharedPtr _msg){
    ...
    traj_status_queue.push(_msg->data); 
}
...
void PaPEvaluationNode::publishEvaluation() {
    // all data is published with the same timestamp for easier evaluation of the data
    ...
    // eef position error from setpoint with sign in x,y,z 
    ...
    // eef attitude error from setpoint with sign in roll, pitch, yaw 
    ...
    if (std::isnan(intersec(0))) {
        ...
        msg_placed_pos_.vector.z = intersec.z(); // should be 0
    ...
    msg_gripped_angle_.vector.x = 0.0;
    ...
    eef_pos_pub_->publish(msg_eef_pos_); //const
    eef_pos_setpoint_pub_->publish(msg_eef_pos_setpoint_); //const
    eef_pos_vec_error_pub_->publish(msg_eef_pos_vec_err_); //const
    eef_att_vec_error_pub_->publish(msg_eef_att_vec_err_); //const
    ...
    eef_pos_error_pub_->publish(msg_eef_pos_err_); //const
    eef_att_error_pub_->publish(msg_eef_att_err_); //const
