...
void PaPEvaluationNode::publishEvaluation() {
    ...
    Eigen::Vector3d euler_eef = eef_att_.toRotationMatrix().eulerAngles(0, 1, 2);
    ...
    if (std::isnan(intersec(0))) {
        ...
        msg_placed_pos_.vector.z = intersec.z();
    ...
    Eigen::Vector3d err_vec = cylinder_att_.toRotationMatrix().transpose() * (eef_pos_ - cylinder_pos_);
    ...
    eef_pos_pub_->publish(msg_eef_pos_);
    eef_pos_setpoint_pub_->publish(msg_eef_pos_setpoint_);
    eef_pos_vec_error_pub_->publish(msg_eef_pos_vec_err_);
    eef_att_vec_error_pub_->publish(msg_eef_att_vec_err_);
    ...
    eef_pos_error_pub_->publish(msg_eef_pos_err_);
    eef_att_error_pub_->publish(msg_eef_att_err_);
