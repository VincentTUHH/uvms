...
void uvms_kinematics::skew(const Eigen::Ref<const Eigen::Vector3d> &x,
                           Eigen::Ref<Eigen::Matrix3d> x_tilde) {
  x_tilde << 0, -x(2), x(1), x(2), 0, -x(0), -x(1), x(0), 0;
}

void uvms_kinematics::quaternionError(const Eigen::Quaterniond &des,
  ...
  out =
      state.w() * des.vec() - des.w() * state.vec() - v_des_tilde * state.vec();
...
Kinematics::Kinematics(const std::vector<param_utils::TFParam> &tf_param)
    : auv_pos_(states_.segment<3>(0)),
      ...
      R_B_0_(rot_utils::rpy_to_matrix(tf_param[0].basicTF.r,
                                      tf_param[0].basicTF.p,
                                      tf_param[0].basicTF.y)) {
  states_(2) = 1.0;
  manipulator_kin_ = new alpha_kinematics::Kinematics(tf_param);
}

Kinematics::Kinematics(Eigen::Matrix<double, n_active_joints + 1, 4> &DH_table,
...
void Kinematics::update(const StateVector &q, const Eigen::Vector3d &pos,
                        const Eigen::Quaterniond &att) {
  states_.segment<3>(0) = pos;
  rot_utils::quat_to_rpy(att, states_.segment<3>(3));
  // states_.segment<3>(3) = att.toRotationMatrix().eulerAngles(2, 1,
  // 0).reverse();
...
void Kinematics::getJacobian([[maybe_unused]] const StateVector &q,
  ...
  getTransformationBodyRatesEuler(auv_att_(0), auv_att_(1), auv_att_(2),
                                  J.block<3, 3>(3, 3));
...
const Eigen::Quaterniond &Kinematics::getEefAttitude() { return att_eef_; }
...
const Eigen::Ref<const Eigen::Vector3d> Kinematics::getLinkBaseRefPosition(
    const int &idx) {
  return manipulator_kin_->getLinkPosition(idx);
}

void Kinematics::getLinkBaseRefPositionJacobian(
    const int &idx, Eigen::Matrix<double, 3, n_states_uvms> &J) {
  manipulator_kin_->getLinkPositionJacobian(idx,
                                            J.block<3, n_active_joints>(0, 6));
}
