...
void UVMSTrajGenPickPlace::sendSetpoint() {
  ...
  switch (traj_status_) {
    ...
    case TrajStatus::approaching_initial_manipulator_pose:
      ...
      // to eef's current pose is published such that estimation_drift_watchdog will not force
      // shutdown
    ...
    case TrajStatus::approaching_goal: {
      ...
      // calculate quaternion error / attitude error of eef:
      ...
      Eigen::Vector3d att_error = att_.w() * goal_att_.vec() - 
                                  ...
                                  goal_att_tilde * att_.vec(); // skew matrix replaces cross prodcut with vector matrix multiplication
...
void UVMSTrajGenPickPlace::updateGoal(
  ...
  if (!received_goal_) {
    ...
    if (msg->number == static_cast<uint32_t>(old_goal_number_ + 1)) {
      old_goal_number_++;
      received_goal_ = true;
    } else {
      return;
